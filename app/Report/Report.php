...
class Report
{
    ...
    public function PDF(\stdClass $data, object $db)
    {
        ...
        foreach ($db as $info) {
            foreach ($data->{self::FIELDS} as $KEY => $INFO) {
                if ($INFO->{self::ACTIVE} === true) {
                    ...
                    $pdf->Cell($INFO->{self::WIDTH}, $data->{self::INFO}->{self::HEIGHT}, utf8_decode($info->{$KEY}), true, 0, false);
                }
            }
        ...
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell($width, $data->{self::INFO}->{self::HEIGHT}, 'GENERATED BY @' . utf8_decode(\App\Helpers\Utils::user()->name), true, 0, 'C');
